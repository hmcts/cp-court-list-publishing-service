...
public class CourtListPublishStatusService {
    ...
    @Transactional
    public CourtListPublishResponse updateError(final UUID courtListId, final String errorMessage) {
        validateCourtListId(courtListId);
        LOG.atDebug().log("Updating error message for court list ID: {}", courtListId);
        
        CourtListPublishStatusEntity entity = findEntityOrThrow(courtListId);
    ...
    @Transactional
    public List<CourtListPublishResponse> findByPublishStatus(final String publishStatus) {
        validatePublishStatus(publishStatus);
        LOG.atDebug().log("Fetching court list publish statuses for status: {}", publishStatus);
        return repository.findByPublishStatus(publishStatus).stream()
                .map(CourtListPublishResponse::from)
                .collect(Collectors.toList());
    }

    @Transactional
    public List<CourtListPublishResponse> findByCourtCentreIdAndPublishStatus(
            final UUID courtCentreId,
            final String publishStatus) {
        validateCourtCentreId(courtCentreId);
        validatePublishStatus(publishStatus);
        ...
        return repository.findByCourtCentreIdAndPublishStatus(courtCentreId, publishStatus).stream()
                .map(CourtListPublishResponse::from)
                .collect(Collectors.toList());
    }
    ...
    public void deleteByCourtListId(final UUID courtListId) {
        ...
        if (!repository.existsById(courtListId)) {
            LOG.atWarn().log("Court list publish status not found for ID: {}", courtListId);
            throw new ResponseStatusException(HttpStatus.NOT_FOUND, ERROR_ENTITY_NOT_FOUND);
        }
    ...
    private CourtListPublishStatusEntity findEntityOrThrow(final UUID courtListId) {
        CourtListPublishStatusEntity entity = repository.getByCourtListId(courtListId);
        if (entity == null) {
            LOG.atWarn().log("Court list publish status not found for ID: {}", courtListId);
            throw new ResponseStatusException(HttpStatus.NOT_FOUND, ERROR_ENTITY_NOT_FOUND);
